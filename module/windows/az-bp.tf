...
resource "azurerm_backup_protected_vm" "bpvm" {
  ...
  count               = var.add_to_backup == true && var.enable_legacy_vm_module == false ? 1 : 0
  resource_group_name = var.recovery_vault_resource_group
  recovery_vault_name = var.recovery_vault_name
...
resource "azurerm_backup_protected_vm" "bpvm_legacy" {
  depends_on = [azurerm_virtual_machine.vm, data.azurerm_backup_policy_vm.default_bkup_policy]
  count      = var.add_to_backup == true && var.enable_legacy_vm_module == true ? 1 : 0

  resource_group_name = var.recovery_vault_resource_group
  recovery_vault_name = var.recovery_vault_name
  source_vm_id        = azurerm_virtual_machine.vm[0].id
  backup_policy_id    = azurerm_backup_policy_vm.bkup_policy[0].id
}
